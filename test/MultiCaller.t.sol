...
contract MultiCallerTest is Test {
    ...
    function test_record_call() public {
        multiCaller.recordCall();

        console.log("last_caller 1", multiCaller.lastCaller());

        vm.startPrank(address(1));

        multiCaller.recordCall();

        console.log("last_caller 2", multiCaller.lastCaller());

        // vm.prank(address(2));

        multiCaller.recordCall();

        console.log("last_caller 3", multiCaller.lastCaller());
        vm.stopPrank();
        multiCaller.recordCall();

        console.log("last_caller 4", multiCaller.lastCaller());
        multiCaller.recordCall();

        console.log("last_caller 5", multiCaller.lastCaller());
    }
